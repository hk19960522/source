...
void GAME_OBJ::initParticleSystem() {
	mPSystem = static_cast<SceneNode*>(
		mSceneNode->createChild());
	//ParticleSystem* p = mSceneMgr->createParticleSystem(
	//		"explosion", "Examples/JetEngine1");
	ParticleSystem* p = mSceneMgr->createParticleSystem(
		mPSystemName, "Examples/GreenyNimbus");

	ParticleEmitter *e = p->getEmitter(0);
	e->setEnabled(false);
	mPSystem->attachObject(p);
	mPSystem->setPosition(Vector3::ZERO);
	mPSystem->setVisible(false);
}

void GAME_OBJ::setOffParticleSystem(bool state) 
{
	mPSystem->setVisible(state);
	MovableObject *s = mPSystem->getAttachedObject(mPSystemName);
